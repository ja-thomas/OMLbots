...
makeRandomTable = function(task.id.filter, flow.id, lrn.ps.sets, n, tbl.metaFeatures){
...
makeMeasureTimePrediction = function(measure.name, flow.id, task.id, lrn.par.set, n, 
  tbl.results, tbl.hypPars, tbl.metaFeatures)){
    
  #train mlr model on full table for measure and runtime
  task.data = makeBotTable(measure.name, flow.id, tbl.results, tbl.hypPars, tbl.metaFeatures)
  ...
  pred.data = makeRandomTable(task.id, flow.id, lrn.par.set, n, tbl.metaFeatures)
